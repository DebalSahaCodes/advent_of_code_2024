...
if __name__=='__main__':
    ...
    for rob in all_rob_dat:
        ...
        str1 = "Checking rob pos: " + r_pos.get_str()
        if is_quad_1_pos(r_pos, xMax, yMax):
            n_quad_1+=1
            str1 += " is in Quad-1"
        elif is_quad_2_pos(r_pos, xMax, yMax):
            n_quad_2+=1
            str1 += " is in Quad-2"
        elif is_quad_3_pos(r_pos, xMax, yMax):
            n_quad_3+=1
            str1 += " is in Quad-3"
        elif is_quad_4_pos(r_pos, xMax, yMax):
            n_quad_4+=1
            str1 += " is in Quad-4"
        else: # at quad boundary so skip
            str1 += " is at Quad Bdry"
            continue
        print(str1)
    safety_factor = n_quad_1 * n_quad_2 * n_quad_3 * n_quad_4
    print("SAFETY FACOTR: ", safety_factor)
