...
std::map<std::pair<int,int>,char> g_maze;
...
LTYPE get_next(const RTYPE& given_v)
{
    ...
    const char c_p1 = g_maze[std::make_pair(std::get<0>(p1),std::get<1>(p1))];
    const bool found_E = c_p1=='E';
    const bool found_H = c_p1=='#';
    const bool found_MIN = !g_scores.empty() && score >= g_scores.front();
